        ...
        if LoginOrSignUp.selectedSegmentIndex == 0 {
            ...
            Auth.auth().createUser(withEmail: email.text!, password: password.text!, completion: { (user, error) in
                ...
                storageRef.putData(pictureData!, metadata: nil, completion: { (metadata, error) in
                    ...
                    ref.child("users").child((user?.uid)!).setValue(["userName": self.userName.text!, "email": self.email.text!, "capScore": 0, "fullName": "", "profilePicture": profilePictureURL!, "photos": photoPaths, "followers": followers, "following": following, "followRequests": followRequests])
                })
            })
            self.performSegue(withIdentifier: "loginSegue", sender: self)
        }
